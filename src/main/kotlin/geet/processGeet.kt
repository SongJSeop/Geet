...
fun processGeet(commandLines: Array<String>): Unit {
    ...
    when (commandLines[0]) {
        ...
        "switch" -> geetSwitch(commandLines)
        "merge" -> geetMerge(commandLines)
        "restore" -> geetRestore(commandLines)
...
fun guideGeet(): Unit {
    ...
    println("|  branch  |  브랜치를 생성하거나, 브랜치 목록을 출력하거나, 브랜치를 삭제합니다.  |")
    println("|  switch  |  브랜치를 변경합니다.  |")
    println("|  merge  |  브랜치를 병합합니다.  |")
    println("|  restore  |  파일을 이전 커밋 상태로 복원합니다.  |")
